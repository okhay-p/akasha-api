...
func GenerateLessonPlan(req req_structs.GenerateTopicReqBody, file *multipart.FileHeader) (LessonPlan, error) {
	...
	if file != nil {
		...
		parts = append(parts, genai.Text("Use the attached PDF as part of the user content."))
	}
	...
	resp, err := ai.GeminiModel.GenerateContent(ai.Ctx, parts...)
	if err != nil {
		log.Println(err)
		return plan, err
	}
